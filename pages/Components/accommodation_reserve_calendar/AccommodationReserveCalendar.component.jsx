...
class AccommodationReserveCalendar extends React.Component {
    ...
    checkDateIsContained = (day) => {
        ...
        const isContained = this.state.selectedDaysArray.filter(x => x[0] == date)[0]
    ...
    fillSelectedDate = () => {
        const duration = this.state.days.slice(this.state.firstDate, this.state.secondDate + 1)
        const selectedDate = []
        duration.forEach(oneRow => {
            const m = moment(`${this.state.year}/${this.state.month}/${oneRow.day}`, 'jYYYY/jMM/jDD')
