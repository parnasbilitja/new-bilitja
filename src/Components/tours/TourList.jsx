      ...
      filteredData = filteredData.filter(
        (data) =>
          (!filter.airline.departure ||
            data?.flightData?.departure_flight?.airline ===
              filter.airline.departure) &&
          (!filter.airline.return ||
            data?.flightData.return_flight?.airline === filter.airline.return)
      );
    }

    if (filter.nights) {
      filteredData = filteredData.filter(
        (data) => data.night_num === filter.nights
      );
    }

    if (filter.tour_id !== "") {
      filteredData = filteredData.filter((data) =>
        data?.id?.toString()?.includes(filter?.tour_id?.toString())
      );
    }
        ...
        <div>
          ...
          <div style={{ marginTop: "5rem" }}>
            {isLoading || toursLoading ? (
                      ...
                      <Loading title={`  درحال جستجو تورهای انتخابی شما هستیم .....`}/>
                  ...
                  >
                    ...
                    <div className={styles["filter_item"]}>
                      <div className={styles["title"]}>
                        ...
                        <p className={`${styles["title"]} font-bold-iransanse`}>
                          {" "}
                          جستجو براساس شناسه تور
                        </p>
                      </div>

                      <div className={"mt-3"}>
                        <input
                          value={filter.tour_id}
