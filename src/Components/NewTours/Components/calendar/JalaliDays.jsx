...
class JalaliDays extends React.Component {
  ...
  getDaysNextMonth = () => {
    ...
    const month =
      parseInt(this.state.month) + 1 > 12 ? 1 : parseInt(this.state.month) + 1;
  ...
  evDateByDestAndOrg = (x) => {
    const year =
      parseInt(this.state.month) > 12
        ...
        : parseInt(this.state.year);
    const month =
      parseInt(this.state.month) > 12 ? 1 : parseInt(this.state.month);
...
export default connect(mapStatesToProps, mapDispatchesToProps)(JalaliDays);
