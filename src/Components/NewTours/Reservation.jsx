...
const Reservation = (props) => {
        ...
        passengers: room.passengers.map((pass) => {
          return {
            ...
            birth_day:
              pass.birth_day === ""
                ? ""
                : +getYear(pass.birth_day) > 1900
                ? pass.birth_day
                : moment(pass.birth_day, "jYYYY/jMM/jDD").format("YYYY-MM-DD"),
            expired_passport:
              pass.expired_passport === ""
                ? ""
                : +getYear(pass.expired_passport) > 1900
                ? pass.expired_passport
                : moment(pass.expired_passport, "jYYYY/jMM/jDD").format(
                    "YYYY-MM-DD"
                  ),
          };
        }),
  ...
  useEffect(() => {
    ...
    setReserverData({
      ...reserverData,
      reserver_phone: props.hotelDet?.data?.information.reserver.phone,
    });


    const selected_roomsReformat = props.hotelDet?.data?.selected_rooms.map(
      (selectedRoom) => {
        return {
          ...selectedRoom,
          passengers: selectedRoom.passengers.map((passenger, i) => {
            return {
              ...
              pass_id:
                i +
                passenger.type +
                (passenger.child_type ? passenger.child_type : ""),
            };
          }),
        };
      }
  ...
  const reserveTour = () => {
    ...
    axios
      .post(
        `https://api.hotelobilit.com/api/v3/reserves/${router.query.ref_code}`,
        {
      ...
      .then((res) => {
        setLoading(false)
        router.push(
          `/tour/reserve/reserveconfirmation/${hotel_id}?ref_code=${router.query.ref_code}&staycount=${props.stayCount}`
        );
          ...
          <div className={styles["main-reserve"]}>
              ...
              <div className={styles["detail-box-fix-user-reservation"]}>
                <div className={styles["p-detail-reservation"]}>
                  ...
                  <div className={styles["paymentbtn"]}>
                    ...
                    >
                    {loading ?'لطفا منتظر بمانبد...':'تایید اطلاعات'}
                    </button>
                  </div>
                </div>
              </div>
            ...
            <div className={styles["tourDet-container"]}>
              <div className={styles["selected-hotel_conatiner"]}>
                ...
                >
                  {startBuilder(+props.hotelDet?.data?.hotel?.stars).map(
                    (x) => {
                      return (
                        <div
                          style={{
                ...
                <div className={styles["selected-hotel"]}>
                  <div className={styles["selected-hotel-names"]}>
                    <h2>
                      {props.hotelDet?.data?.hotel?.is_domestic
                        ? props.hotelDet?.data?.hotel?.title
                        : props.hotelDet?.data?.hotel?.titleEn}
                    </h2>
                    <p>
                      {" "}
                      {props.hotelDet?.data?.hotel?.is_domestic
                  ...
                  <>
                    <div className={styles["entext"]}>
                      ...
                      <p style={{ fontWeight: "500", fontSize: "12px" }}>
                        {MiladiToJalaliConvertor(
                          props.hotelDet?.data?.hotel?.checkin
                        )}
                      </p>
                      <p>
                        {getDayInPersian(
                          moment(props.hotelDet?.data?.hotel?.checkin).format(
                            "dddd"
                          )
                        )}
                      </p>
                    ...
                    <div className={styles["entext"]}>
                      ...
                      <p style={{ fontWeight: "500", fontSize: "12px" }}>
                        {MiladiToJalaliConvertor(
                          props.hotelDet?.data?.hotel?.checkout
                        )}
                      </p>
                      <p>
                        {getDayInPersian(
                          moment(props.hotelDet?.data?.hotel?.checkout).format(
                            "dddd"
                          )
                        )}
                      </p>
            ...
            <div className={styles["box-top-box-reserve"]}>
              ...
              {props.hotelDet.data?.information?.expired_at ? (
                <CountDownTimer
                  hoteldata={props.hotelDet.data}
                  exp_time={props.hotelDet.data?.information?.expired_at}
                />
              ) : (
                  ...
                  <div className={styles["item-form"]}>
                    ...
                    {err.errors && (
                      ...
                      >
                        ...
                        {err.errors &&
                        reserverData.reserver_id_code.length < 10 ? (
                          <p>کد ملی نباید کمتر از 10 رقم باشد.</p>
                        ) : null}
                      </div>
                    )}
                  </div>
              ...
              <div className={styles["rules"]}>
                ...
                >
                  <p>
                    {" "}
                    ثبت درخواست به منزله پذیرش تمام <span>
                      قوانین و مقررات
                    </span>{" "}
                    مرتبط با سایت هتل و بلیط و پکیجهای این تور می باشد
                  </p>
                </div>
              </div>
            </div>
