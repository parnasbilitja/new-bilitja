...
class ManagementVilaSetPriceCalendar extends React.Component {
    ...
    getDays = () => {
        ...
        const d1 = `${this.state.year}${('0' + this.state.month).slice(-2)}${('0' + arrayOfdays[0]).slice(-2)}`
        const d2 = `${this.state.year}${('0' + this.state.month).slice(-2)}${('0' + arrayOfdays[arrayOfdays.length - 1]).slice(-2)}`
        fetch(`${globals.baseUrl}bj/datePrice/view/${this.props.accommodationId}/1/${d1}/${d2}`)
            .then(res => res.json())
            .then(data => {
                this.setState({
    ...
    checkDateIsContained = (day) => {
        ...
        const isContained = this.props.selectedDaysArray.filter(x => x == date)[0]
    ...
    render() {
        ...
        return (
            <div className="accommodation-calendar-reserve">
                <div>
                    <div className="row calendar-header" style={{ margin: "0" }}>
                    ...
                    <div>
                        <div className="accommodation-calendar-reserve-day-container">
                            ...
                            <div className="font-size-13 color-black">جمعه</div>
                            {
                                this.state.days.map(x => {
                                    ...
                                    let price
                                    try {
                                        price = this.state.daysWithPrice.filter(oneDay => oneDay.DateM == `${this.state.year}/${this.state.month}/${x}`)[0].Price
                                    } catch (error) {
                                        price = "خالی"
                                    }
                                    ...
                                    return (
                                        x != undefined ?
                                            <div className={`manager-calendar-item ${compareToToday} ${isSelected}`} onClick={() => {
