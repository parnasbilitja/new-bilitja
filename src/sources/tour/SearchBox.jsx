...
const SearchBox = ({ state, setState, toursHandler, executeScroll }) => {
  ...
  useEffect(() => {
    if (toursData.selectedOrigin) {
      let foundOrg = origins.filter(
        (org) => org.code === toursData.selectedOrigin.code
      )[0]?.destinations;
  ...
  useEffect(() => {
    let foundDate = AvDates?.filter(
      (date) => date?.date === toursData.selectedDate?.miladiDate
    );
  ...
  const SearchBoxSteps = (step) => {
    switch (step) {
      ...
      case "date":
        if (
          toursData.selectedDestination.name &&
          toursData.selectedDestination.code
        ) {
          dispatch(setSearchStep(step));
        } else {
      ...
      case "night":
        if (
          toursData.selectedDate.miladiDate &&
          toursData.selectedDate.persianDate
        ) {
          dispatch(setSearchStep(step));
        } else {
        ...
        <div className={styles["tour_type"]}>
          <div
            className={`${styles["type_hotel"]} ${styles["type"]} ${
              toursData.tour_type === "hotel" ? styles["type_active"] : ""
            }`}
            onClick={() => dispatch(setTourType("hotel"))}
          >
            <p className="p-0 m-0">هتلی </p>
          </div>
          <div
            className={`${styles["type_package"]} ${styles["type"]}  ${
              toursData.tour_type === "package" ? styles["type_active"] : ""
            }`}
            onClick={() => dispatch(setTourType("package"))}
          >
            <p className="p-0 m-0">پکیجی</p>
          </div>
        </div>
      </div>
      <div
        className={styles["search_box_container"]}
        style={{ padding: "0 10px", columnGap: "12px" }}
      >
        <div className={"w-100 position-relative"}>
          <div className={` form-input-border ${styles["prs-input"]} `}>
            ...
            <div
              className={
                "d-flex align-items-center justify-content-between gap-2 ml-2 px-2"
              }
            >
              {toursData.selectedOrigin.name && (
                <svg
        ...
        <div className={"w-100 position-relative"}>
          ...
          >
            <PrimaryTextInputMobile
              value={
                toursData.selectedNight
                  ? toursData.selectedNight + " " + "شب"
                  : ""
              }
              name={"slug"}
              onFocus={handleFocus}
        ...
        <div className="col-12 without-focus px-0">
          <button
            className={`${styles1["primary-button"]} px-0 soc01`}
            style={{
              width: "100%",
              height: "55px",
              marginTop: "7px",
            ...
            onClick={() => {
              ...
              if (hasSearchCriteria) {
                ...
                Err("لطفا اطلاعات تور خود را وارد کنید.");
              }
            }}
          >
            {isLoading === false ? "جستجو" : "لطفا صبر کنید..."}
          </button>
        </div>
        {toursData.searchboxStep === "date" && (
