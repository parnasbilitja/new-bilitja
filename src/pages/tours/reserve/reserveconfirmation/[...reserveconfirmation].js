...
const ReservationConfirmation = () => {
  ...
  const [confirmationDetail, setConfirmationDetails] = useState({});
  ...
  const getReservedData = (refcode) => {
    ...
    axios
      .get(`https://api.hotelobilit.com/api/v3/reserves/${refcode}`, {
        headers: {
  ...
  const OpenEdit = (reserveId) => {
    ...
    let selectRoom = reservedRoom?.selected_rooms?.filter(
      (reserveRoom) => reserveRoom?.reserve_id === reserveId
    );
  ...
  return (
    <>
          ...
          <div className={styles["message_con"]}>
            <div className={styles["message"]}>
              ...
              <div className={styles["success-message"]}>
                ...
                >
                  {confirmationDetail?.data?.information.status?.id === 471
                    ? " ثبت شما با موفقیت انجام شد . لطفا جهت پرداخت ، یکی از درگاه های زیر را انتخلب نموده وبرروی دکمه پرداخت کلیک کنید."
                    : "ثبت شما با موفقیت انجام شد همکاران ما جهت تایید نهایی با شما تماس می گیرند."}
                </p>
              </div>
              <div
                style={{
              ...
              >
                لطفا رفرنس را جهت پیگیری های بعدی یادداشت فرمایید
              </p>
              ...
              <div className={styles["paymentbtn"]}>
                <button
                  onClick={() => {
                    ErrSuccess(
                      "رزرو شما با موفقیت انجام شد.به صفحه نخست منتقل می شوید. "
                    );
                    setTimeout(() => {
                      router.push("/");
                    }, 3000);
      ...
      {isEdit && (
        <div className={styles["UpadateContainer"]}>
          <div className={styles["updatePass"]}>
            ...
            <div className={styles["updatePass_con"]}>
              <UpdatePassenger
                targetedRoom={targetedRoom[0]}
                setIsEdit={() => setIsEdit(false)}
                EditClickHandler={(passengersArr, room) =>
                  EditClickHandler(passengersArr, room)
                }
                Errs={err}
              />
      ...
      <div className={styles["reserveinfo_container"]}>
        ...
        <div
          className={styles["tourDet-container"]}
          ...
          <div className={styles["selected-hotel_conatiner"]}>
            <div className={styles["selected-hotel"]}>
              <div className={styles["selected-hotel-names"]}>
                <h2>
                  {reservedRoom?.hotel?.is_domestic
                    ? reservedRoom?.hotel?.title
                    : reservedRoom?.hotel?.titleEn}
                </h2>
                <p>
                  {" "}
                  {reservedRoom?.hotel?.is_domestic
            ...
            {!isEmpty(reservedRoom) ? (
              <>
                <div className={styles["entext"]}>
                  ...
                  <p>
                    {getDayInPersian(
                      moment(reservedRoom?.hotel?.checkin).format("dddd")
                    )}
                  </p>
                </div>
                <div className={styles["entext"]}>
                  ...
                  <p>
                    {getDayInPersian(
                      moment(reservedRoom?.hotel?.checkout).format("dddd")
                    )}
                  </p>
                </div>
              </>
        ...
        <div className={styles["rooms"]}>
          <Scrolltoprefresh />
          {reservedRoom?.selected_rooms ? (
            reservedRoom?.selected_rooms?.map((reservedroom) => {
              return (
                <div
                ...
                >
                  ...
                  >
                    <div className={styles["box-room-Det-name"]}>
                      ...
                      <h2>{reservedroom?.info_room?.room_type}</h2>
                    </div>
        ...
        <div className={styles["detail-box-fix-user-reservation"]}>
          <div className={styles["p-detail-reservation"]}>
                ...
                >
                  ...
                  <div style={{ display: "flex" }}>
                    <strong style={{ fontSize: "14px", marginRight: "4px" }}>
                      {reservedRoom?.prices?.total_price &&
                        numberWithCommas(reservedRoom?.prices?.total_price)}
                    </strong>
            ...
            <div className={styles["paymentbtn"]}>
              <button
                onClick={() => {
                  ...
                  if (isLoading) {
                    ...
                    axios
                      .post(
                        `https://api.hotelobilit.com/api/v3/reserves/confirm/${router.query.ref_code}`,
                        {},
                        {
                          headers: {
                            "x-app-key":
                      ...
                      .then((res) => {
                        ...
                        setIsLoading(false);
                        setConfirmationDetails(res.data);
              ...
              >
                {isLoading ? "لطفا منتظر بنانید" : " پرداخت با کارت شتاب"}{" "}
              </button>
              <p
                onClick={() => {
                  router.push("/tour");
